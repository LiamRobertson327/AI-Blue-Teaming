...
Options:
  ...
  --start-server   OPTIONAL: try to start the example server (uvicorn) as a subprocess.
                   Use only in local dev. If used, the script will try to stop it at the end.
  --timeout     HTTP timeout seconds (default: 5)
  --out         Output evidence JSON path (default: redteam/results/collected_evidence_<ts>.json)
Notes:
  - This script is safe and only calls the local demo endpoints in the starter repo.
  - Ensure you run this in the project root where `run_instructions.md` and `app/` are present.
\"\"\"
...
def start_uvicorn(app_module: str = \"app.main:app\", port: int = 8000):
    # Starts uvicorn in a subprocess; returns the subprocess object.
...
def main():
    ...
    try:
        if args.start_server:
            proc = start_uvicorn(port=int(args.base_url.rsplit(':',1)[-1]) if ':' in args.base_url else 8000)
            print(\"Waiting 1.5s for server to warm up...\")
            time.sleep(1.5)
        ...
        if not health.get('ok'):
            print(\"Health check failed:\", health.get('error'), file=sys.stderr)
            # continue to try endpoints anyway; maybe server returns /tasks without /health
        else:
            print(\"Health OK ->\", health.get('status_code'))

        # 1) Run a normal workflow: POST /tasks
        print(\"Running normal workflow: POST /tasks ...\")
        ...
        print(\"Task response:\", task_res.get('json') if task_res.get('ok') else task_res.get('error'))

        # 2) Run safe red-team simulation: POST /tests/rt-01
        print(\"Running red-team simulation: POST /tests/rt-01 ...\")
        rt_res = run_rt01(args.base_url, timeout=max(args.timeout, 10))
        evidence['steps'].append({\"step\": \"run_rt01\", \"result\": rt_res})
        print(\"RT-01 response:\", rt_res.get('json') if rt_res.get('ok') else rt_res.get('error'))

        # 3) Fetch logs
        print(\"Fetching logs: GET /logs ...\")
        ...
        if logs_res.get('ok') and isinstance(logs_res.get('json'), list):
            # Save a sanitized subset of logs
            safe_logs = []
            for item in logs_res['json']:
                # redact any fields that look sensitive (basic heuristics)
                redacted = dict(item)
                for k in list(redacted.keys()):
                    if k.lower().find('secret') != -1 or k.lower().find('token') != -1:
                        redacted[k] = '<REDACTED>'
